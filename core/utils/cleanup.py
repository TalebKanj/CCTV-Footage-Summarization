...
def prune_old_logs(logs_path: str = "data/runtime_logs.json") -> int:
    """Prune runtime logs older than retention_days. Returns number pruned."""
    db = _load_db(logs_path)
    retention_days = int(db.get("retention_days", RETENTION_DAYS))
    cutoff = (datetime.now() - timedelta(days=retention_days)).isoformat()

    original_count = len(db.get("logs", []))
    db["logs"] = [log for log in db.get("logs", []) if log.get("timestamp", "") > cutoff]
...
def add_log(message: str, level: str = "INFO", logs_path: str = "data/runtime_logs.json") -> None:
    db = _load_db(logs_path)
...
def get_recent_logs(limit: int = 100, logs_path: str = "data/runtime_logs.json") -> List[Dict]:
    db = _load_db(logs_path)
    logs = db.get("logs", [])
